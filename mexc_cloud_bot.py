...
# 開單前觀察
OBSERVATION_SECONDS = 15  # 開單前觀察 15 秒
...
class FixedGrid:
    ...
    def __init__(self, grid_id, min_price, max_price, capital):
        ...
        # 止損止盈：超出震盪區間
        self.lower_close = round(self.min_price - GRID_TICK, 4)
        self.upper_close = round(self.max_price + GRID_TICK, 4)
...
class FixedGridBot:
    ...
    def _observe_price(self):
        ...
        logging.info(f"觀察結果: 震盪區間 ${min_price:.4f} ~ ${max_price:.4f}")
    ...
    def try_observe(self):
        ...
        # 觀察震盪區間
        min_price, max_price = self._observe_price()
        
        if min_price is None or max_price is None:
            return
        
        # 創建網格
        self._create_grid(min_price, max_price)
    
    def _create_grid(self, min_price, max_price):
        """創建網格"""
    ...
    def display_status(self):
        ...
        if self.current_grid and self.current_grid.active:
            grid = self.current_grid
            logging.info("📋 當前網格:")
            logging.info(f"  {grid.id} @ ${grid.min_price:.4f}~${grid.max_price:.4f}")
